...
int len_decoder(char *recv_buf,int recv_len,int head_len)
{
    ...
    while(i<recv_len)
    {
        ...
        // {
        //     head_len = i+4;
        //     ESP_LOGI(TAG, "head_len = %d", head_len);
        // }
        i++;
    }
    //ESP_LOG_BUFFER_HEX(TAG, recv_buf, recv_len);
    return length;
}
...
void JPEGDraw(JPEGDRAW *pDraw)
{
    lcd_write_bitmap(pDraw->x,pDraw->y,pDraw->iWidth,pDraw->iHeight,pDraw->pPixels);
} 
...
{
    int rc;
    rc = JPEG_openRAM(&jpg, mjpegbuffer, size, JPEGDraw);
    if (rc)
    {
...
static void http_rest_with_url(void)
{
    ...
    //ESP_LOG_BUFFER_CHAR  ESP_LOG_BUFFER_HEX
    // POST
    const char *post_data = "{\"field1\":\"value1\"}"; //"GET /?action=snapshot HTTP/1.1\r\n";
    esp_http_client_set_url(client, "http://httpbin.org/post"); // //"http://172.16.171.97:8080/?action=snapshot"
    esp_http_client_set_method(client, HTTP_METHOD_POST);
    esp_http_client_set_header(client, "Content-Type", "application/json");//"image/jpeg"
...
{
    esp_http_client_config_t config = {
        .host = "http://172.16.171.97:8080/",//"httpbin.org",
        .path = "/get",
    ...
    // POST
    const char *post_data = "GET example.jpg HTTP/1.1\r\n\r\n";//"field1=value1&field2=value2";
    esp_http_client_set_url(client, "/example.jpg");
    esp_http_client_set_method(client, HTTP_METHOD_POST);
    esp_http_client_set_post_field(client, post_data, strlen(post_data));
    err = esp_http_client_perform(client);
    if (err == ESP_OK) {
...
{
    esp_http_client_config_t config = {
        .url = "http://172.16.171.97:8080/example.jpg",//"http://i2.hdslb.com/bfs/face/bce14f5e3af4bca480fc7de227986ba304507078.jpg",//"http://httpbin.org/stream-bytes/8912",
        .event_handler = _http_event_handler,
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
    esp_err_t err = esp_http_client_perform(client);
...
static void http_native_request(void)
{
    ...
    esp_http_client_config_t config = {
        .url = "http://i2.hdslb.com/bfs/face/bce14f5e3af4bca480fc7de227986ba304507078.jpg",//"http://172.16.171.97:8080/example.jpg",//"http://httpbin.org/get",
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
    ...
    esp_http_client_set_url(client, "http://172.16.171.97:8080/?action=snapshot");//"http://httpbin.org/post"
    esp_http_client_set_method(client, HTTP_METHOD_POST);
    esp_http_client_set_header(client, "Content-Type","application/json");//"image/jpeg"
...
void get_face_jpg(void)
{
 
	static esp_http_client_config_t config = {
		.url = "http://172.16.171.97:8080/?action=snapshot",//这里是你的B站头像链接
		.event_handler = _http_event_handler,//face_http_event_handler,
		.buffer_size = 3 * 1024,
		.timeout_ms = 4000,
		.path = "/get",
		.transport_type = HTTP_TRANSPORT_OVER_TCP,
